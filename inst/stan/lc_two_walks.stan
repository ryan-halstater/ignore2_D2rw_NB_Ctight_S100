...
parameters {
  ...
  vector[J] a;                          // alpha_x
  simplex[J] b1;                        // beta_x, strictly positive and sums to 1
  simplex[J] b2;                        // beta_x, strictly positive and sums to 1
...
model {
  ...
  target += dirichlet_lpdf(b1|rep_vector(1, J));  
  target += dirichlet_lpdf(b2|rep_vector(1, J));     // Prior on betas
  target += exponential_lpdf(sigma1 | 100);         // Exponential prior for sigma
  target += exponential_lpdf(sigma2 | 100); 
